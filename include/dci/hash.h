...
public:
    ...
    JSDLSHasher(LSHasherSettings settings, double r, uint64_t seed=0): settings_(settings) {
        ...
        randproj_ = blaze::generate(nh, nd, [&](size_t, size_t){return gen(mt);}) / r;
...
template<typename FT=double, bool SO=blaze::rowMajor, bool use_offsets=true>
class ClippedL1LSHasher: public PStableLSHasher<ThresholdedCauchyDistribution, FT, SO, use_offsets> {
public:
    using super = PStableLSHasher<ThresholdedCauchyDistribution, FT, SO, use_offsets>;
    template<typename...Args>
...
template<typename Hasher, typename IT=::std::uint32_t, typename KT=uint64_t, template<typename...> class HashMap=std::unordered_map>
struct LSHTable {
    using ElementType = typename Hasher::ElementType;
    const Hasher hasher_;
    std::unique_ptr<HashMap<KT, std::vector<IT>>[]> tables_;
    const unsigned nh_;
...
public:
    ...
    LSHTable(Args &&...args): hasher_(std::forward<Args>(args)...),
                              tables_(new HashMap<KT, std::vector<IT>>[hasher_.l()]),
                              nh_(hasher_.nh()),
    ...
    template<typename VT, bool OSO>
    HashMap<IT, unsigned> query(const blaze::Vector<VT, OSO> &query) const {
        auto hv = evaluate(hash(query));
        HashMap<IT, unsigned> ret;
    ...
    template<typename MT, bool OSO>
    std::vector<HashMap<IT, unsigned>>
    query(const blaze::Matrix<MT, OSO> &query) const {
        ...
        std::vector<HashMap<IT, unsigned>> ret(hv.rows());
        assert(hv.rows() == (~query).rows());
        OMP_PFOR
        for(unsigned j = 0; j < hv.rows(); ++j) {
            auto &map = ret[j];
            typename HashMap<KT, std::vector<IT>>::const_iterator it;
