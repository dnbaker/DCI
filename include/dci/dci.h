#ifndef DYNAMIC_CONTINUOUS_INDEXING_H__
...
INLINE auto perform_lbound(const T &x, ItemType item) {
    using lb::has_lower_bound_mf;
    return perform_lbound(x, item, has_lower_bound_mf<T>());
}

static_assert(lb::has_lower_bound_mf<std::set<int>>::value, "std::set must have lb mf");
...
template<typename FType,
         ...
         template <typename...> class SetTemplate=std::unordered_set,
...
struct Spanner {
    ...
    template<bool TF=blaze::rowVector>
    decltype(auto) view() {
        return blaze::CustomVector<FT, blaze::unaligned, blaze::unpadded, TF>(p_, n_);
    }
    template<bool TF=blaze::rowVector>
...
struct ConstSpanner {
    ...
    template<bool TF=blaze::rowVector>
    decltype(auto) view() const {
        return blaze::CustomVector<const FT, blaze::unaligned, blaze::unpadded, TF>(p_, n_);
    }
    auto data() {return p_;}
    auto data() const {return p_;}
    auto begin() {return p_;}
