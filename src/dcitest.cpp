...
int main(int argc, char *argv[]) {
    ...
    if(inpath) {
        ...
        std::mt19937_64 mt(nd * npoints + k * k2 + std::pow(nd, gamma));
    ...
    std::priority_queue<dci::ProjID<FLOAT_TYPE, int>> pqs;
    ...
    for(unsigned i = 0; i < ls.size(); ++i) {
        ...
        if(v < max_inexact) {
            const auto tmp = dci::ProjID<FLOAT_TYPE, int>(v, i);
            #pragma omp critical
            {
                pqs.push(tmp);
            }
    ...
    std::vector<dci::ProjID<FLOAT_TYPE, int>> exact_topn;
    while(pqs.size()) {
        auto p = pqs.top();
